...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True, o_xyz=None, o_new_xyz=None):
    """
    ...
    if new_xyz is None:
        new_xyz = xyz
    if (o_xyz is None) and (o_new_xyz is None):
        o_xyz = xyz
        o_new_xyz = new_xyz
    if o_new_xyz is None:
        o_new_xyz = o_xyz
    if idx is None:
    ...
    if use_xyz:
        grouped_xyz = o_xyz[idx.view(-1).long(), :].view(m, nsample, 3) # (m, nsample, 3)
        #grouped_xyz = grouping(xyz, idx) # (m, nsample, 3)
        grouped_xyz -= o_new_xyz.unsqueeze(1) # (m, nsample, 3)
