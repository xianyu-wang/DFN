...
def main():
    ...
    now=datetime.now()
    # shell prepare script
    exp_dir = os.path.join(args.log_dir, args.data_name, args.exp_name, now.strftime('%Y_%m_%d_%H_%M_%S'))
    args.save_path = exp_dir
    model_dir = os.path.join(exp_dir, 'model')
    result_dir = os.path.join(exp_dir, 'result')

    os.makedirs(model_dir, exist_ok=True)
    os.makedirs(result_dir, exist_ok=True)
    # shutil.copyfile(__file__, exp_dir)
    args.log_file = os.path.join(exp_dir, 'train.log')
    ...
    if args.data_name == 'stpls':
        STPLS(split='train', data_root=args.data_root, test_area=args.test_area)
        STPLS(split='val', data_root=args.data_root, test_area=args.test_area)
    else:
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch_log % args.save_freq == 0) and main_process():
            filename = args.save_path + '/model/model_{}.pth'.format(epoch)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    scaler = GradScaler()
    with tqdm(total=len(train_loader),ncols=100, bar_format=bar_format, leave=False) as _tqdm:
        _tqdm.set_description('Epoch [{}/{}]'.format(epoch+1, args.epochs))
        for i, (coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
            ...
            _tqdm.set_postfix({'Loss': '{:.4f}'.format(loss_meter.val),
                               'Acc': '{:.2f}'.format(accuracy*100),
                               'Remain': '{}'.format(remain_time)})
            _tqdm.update(1)
...
def validate(val_loader, model, criterion):
    ...
    union_meter = AverageMeter()
    target_meter = AverageMeter()
    bar_format = 'Testing {percentage:3.0f}%|{bar}|{n_fmt}/{total_fmt}[{elapsed}<{remaining}]'

    model.eval()
    end = time.time()
    with tqdm(total=len(val_loader), ncols=100, bar_format=bar_format, leave=False) as _tqdm:
