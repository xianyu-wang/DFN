...
class GatherNeighbor(nn.Module):
    def __init__(self, n_neighbor, n_kernel):
        ...
        self.n_small_neigh = 2 * n_neighbor
        self.n_large_neigh = 4 * n_neighbor
        self.n_kernel = n_kernel
        
    def forward(self, xyz, new_xyz, feat_list, offset, new_offset):
        ...
        offset_small_neigh = (torch.ones(n_point)*self.n_small_neigh).cuda()
        offset_small_neigh = torch.cumsum(offset_small_neigh, 0).int()
        offset_large_neigh = (torch.ones(n_point)*self.n_large_neigh).cuda()
        offset_large_neigh = torch.cumsum(offset_large_neigh, 0).int()
...
class GatherNeighborv2(nn.Module):
    ...
    def forward(self, xyz, new_xyz, feat_list, offset, new_offset, label=None):
        ...
        offset_kernel = (torch.ones(n_point)*self.n_kernel).cuda()
        offset_kernel = torch.cumsum(offset_kernel, 0).int()
        rela_coor = pts_large_neigh - new_xyz.unsqueeze(1).repeat(1, self.n_large_neigh, 1)

        idx_kernel = torch.randperm(self.n_neigh, device='cuda')[:self.n_kernel]
        idx_kernel = idx_kernel.repeat(pts_small_neigh.shape[0], 1)
        kernel_points = self.vector_gather(rela_coor, idx_kernel) # [n_key, n_kernel, 3]

  
        q = self.ln_q(self.fc_q(kernel_points)) # [n_key, n_kernel, d]
        q = torch.matmul(q, self.ln_k(self.fc_k(rela_coor)).transpose(1,2))
        q = torch.matmul(F.softmax(q, 2), self.ln_v(self.fc_v(rela_coor)))
        
        q = torch.tanh(self.bn_1(self.fc_1(q).transpose(1,2))).transpose(1,2)
        q = torch.tanh(self.fc_2(q)) * max_shift.unsqueeze(-1).repeat(1,1,3) # [n_key, n_kernel, 3]
        
        kernel_points = kernel_points + q # [n_keys, n_kernel, 3]
...
class PointTransformerLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.gather_neighbor = GatherNeighbor(nsample, 6)
        
    def forward(self, pxo) -> torch.Tensor:
        p, x, o = pxo  # (n, 3), (n, c), (b)
        c = torch.tensor(x.shape[1])
        ...
        p_r, [x_k, x_v] = self.gather_neighbor(p, p, [x_k, x_v], o, o) # (n, nsample, c)
        p_r = p_r - p.unsqueeze(1)

        for i, layer in enumerate(self.linear_p): p_r = layer(p_r.transpose(1, 2).contiguous()).transpose(1, 2).contiguous() if i == 1 else layer(p_r)    # (n, nsample, c)
        w = torch.matmul(x_q.unsqueeze(1), x_k.transpose(1,2))/torch.sqrt(c) #(n, 1, nsample)
        w = self.softmax(w)
        x_v = torch.matmul(w, x_v + p_r).squeeze(1) 
        
        return x_v


class PointTransformerLayer_original(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        super().__init__()
        self.mid_planes = mid_planes = out_planes // 1
        self.out_planes = out_planes
        ...
        self.gather_neighbor = GatherNeighborv2(nsample, 6)
        
    def forward(self, pxo) -> torch.Tensor:
        p, x, o = pxo  # (n, 3), (n, c), (b)
        x_q, x_k, x_v = self.linear_q(x), self.linear_k(x), self.linear_v(x)  # (n, c)
        
        p_r, [x_k, x_v] = self.gather_neighbor(p, p, [x_k, x_v], o, o)
...
class PointSwinTransformerLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        super().__init__()
        self.mid_planes = mid_planes = out_planes // 1
        self.out_planes = out_planes
...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, blocks=[2, 3, 4, 6, 3], c=6, k=13):
        super().__init__()
        self.c = c
...
class PointTransformerSeg_trainable_scale(nn.Module):
    def __init__(self, block, blocks=[2, 3, 4, 6, 3], c=6, k=13):
        super().__init__()
        self.c = c
        self.in_planes, planes = c, [32, 64, 128, 256, 512]
...
def pointtransformer_seg_repro(**kwargs):
    model = PointTransformerSeg_trainable_scale(PointTransformerBlock, **kwargs)
    return model
